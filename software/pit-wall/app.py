...
RAPIDBIKE_WIFI_HOST = os.environ.get("RAPIDBIKE_WIFI_HOST")
RAPIDBIKE_WIFI_PORT = int(os.environ.get("RAPIDBIKE_WIFI_PORT", "8888"))
...
class SharedState:
    ...
    def update_ecu(
        ...
        with self.lock:
            ...
            self.state.usb_status = f"{source} live"
...
def decode_status_voltage(data: bytes) -> float | None:
    if len(data) != 6:
        return None
    # Verified against Rapid Bike Master's battery display. Keep two decimal
    # places because this is ECU-reported voltage, not a calibrated voltmeter.
    return round(((data[4] << 8) | data[5]) / 400.0, 2)


def socket_request(sock: socket.socket, request: bytes, expected: int, timeout: float) -> bytes:
    sock.settimeout(0.01)
    while True:
        try:
            if not sock.recv(256):
                raise ConnectionError("WiFi bridge closed")
        except TimeoutError:
            break
...
def discover_rapidbike(timeout: float = 0.8) -> tuple[str, int] | None:
    if RAPIDBIKE_WIFI_HOST:
        return RAPIDBIKE_WIFI_HOST, RAPIDBIKE_WIFI_PORT
    ...
    try:
        ...
        try:
            ...
            route_probe.close()
            # iPhone hotspots use 172.20.10.0/28. Home and Android networks
            # are normally /24; send both likely directed broadcasts.
            for prefix in (28, 24):
        ...
        for broadcast in broadcast_addresses:
            discovery.sendto(
                RAPIDBIKE_DISCOVERY_REQUEST,
                (broadcast, RAPIDBIKE_DISCOVERY_PORT),
            )
        data, address = discovery.recvfrom(128)
        text = data.decode("ascii", errors="replace")
        fields = text.split(":")
        if len(fields) != 4 or fields[0] != "WIFI" or fields[1] not in {"STA", "AP"}:
            return None
        return address[0], int(fields[3])
...
class WifiWorker(threading.Thread):
    ...
    def run(self) -> None:
        ...
        while not self.stop_event.is_set():
            try:
                endpoint = self.endpoint or discover_rapidbike()
                if endpoint is None:
                    raise ConnectionError("ESP32 discovery timeout")
                host, port = endpoint
                self.state.update_status(f"WiFi connecting {host}:{port}")
                ...
                while not self.stop_event.is_set():
                    status_data = socket_request(self.sock, STATUS_REQUEST, 6, 0.4)
                    voltage = decode_status_voltage(status_data)
                    data = socket_request(self.sock, RPM_REQUEST, 24, 0.6)
                    decoded = decode_frame(data)
                    if decoded is None:
                        raise RuntimeError(f"invalid RPM response ({len(data)} bytes)")
                ...
                self.state.update_status(f"WiFi retry: {exc}")
                if RAPIDBIKE_WIFI_HOST is None:
                    self.endpoint = None
    ...
    def run(self) -> None:
        try:
            ...
            while not self.stop_event.is_set():
                ...
                status_data = read_for(self.device, 0.05)
                voltage = decode_status_voltage(status_data)
                ...
                if decoded:
                    rpm, throttle, fuel_add = decoded
                    self.state.update_ecu(
                        rpm, throttle, fuel_add, data.hex(" ").upper(), "USB", voltage
                    )
                    snapshot = self.state.latest()
                    if snapshot.recording:
                        self.db_queue.put((time.time(), snapshot))
        ...
        while not self.stop_event.is_set() or not self.db_queue.empty():
            ...
            if buffer and (len(buffer) >= 20 or now - last_commit >= 1.0):
                con.executemany(
                    """
                    INSERT INTO samples (
                        session_id, ts, rpm, battery_voltage, speed_kmh, throttle_percent, fuel_add, gps_age,
...
def latest_payload(s: TelemetryState) -> dict[str, Any]:
    ...
    return {
        ...
        "status": (
            ...
            f" | BAT {'--' if s.battery_voltage is None else f'{s.battery_voltage:.2f}V'}"
        ),
    }
...
def main() -> int:
    ...
    if RAPIDBIKE_LINK == "wifi":
        ...
        if endpoint is not None:
            ecu_worker = WifiWorker(state, db_queue, endpoint)
        else:
            ecu_worker = UsbWorker(state, db_queue)
    else:
